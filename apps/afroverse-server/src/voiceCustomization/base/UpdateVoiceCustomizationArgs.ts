/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { VoiceCustomizationWhereUniqueInput } from "./VoiceCustomizationWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { VoiceCustomizationUpdateInput } from "./VoiceCustomizationUpdateInput";

@ArgsType()
class UpdateVoiceCustomizationArgs {
  @ApiProperty({
    required: true,
    type: () => VoiceCustomizationWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => VoiceCustomizationWhereUniqueInput)
  @Field(() => VoiceCustomizationWhereUniqueInput, { nullable: false })
  where!: VoiceCustomizationWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => VoiceCustomizationUpdateInput,
  })
  @ValidateNested()
  @Type(() => VoiceCustomizationUpdateInput)
  @Field(() => VoiceCustomizationUpdateInput, { nullable: false })
  data!: VoiceCustomizationUpdateInput;
}

export { UpdateVoiceCustomizationArgs as UpdateVoiceCustomizationArgs };
